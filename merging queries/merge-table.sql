...
CREATE PROCEDURE mergeTable(
    ...
    IN pk_column VARCHAR(100),
    IN fk_column VARCHAR(100)
) 
BEGIN
...
/* Create the full statement to execute */
set @tmpinsert = concat('insert into tmp_table (name)select distinct name from ',@source_table_name,';');
...
deallocate prepare DynamicStatement;
    ...
    /* Create the full statement to execute */
set @insertsrcstmt = concat ('insert into ',@source_table_name,'(id,name) select id,name  from tmp_table ;');
/* Prepare and execute the statement that was built */
